...
from detectron2.projects.generation.engine import (
    hooks,
    AMPGANTrainer,
    SimpleGANTrainer
)
...
def do_train(args, cfg):
    ...
    trainer = (
        AMPGANTrainer(model, train_loader, grad_clipper=cfg.train.amp.grad_clipper)
        if cfg.train.amp.enabled
        else SimpleGANTrainer(model, train_loader, optim)
    )
    ...
    trainer.register_hooks(
        [
            ...
            hooks.LRSchedulerGAN(scheduler=instantiate(cfg.lr_scheduler)),
...
if __name__ == "__main__":
    ...
    args.config_file = './configs/baseline_GAN.py'
    # args.resume = True
    # args.eval_only = True
    launch(
        main,
