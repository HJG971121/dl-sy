...
def export_tracing(torch_model, inputs):
    ...
    def eval_wrapper(inputs):
        ...
        unused in deployment but needed for 1. We add it manually here.
        """
        input = inputs[0]
...
if __name__ == "__main__":
    ...
    # run 1 with the converted model
    if args.run_eval:
        assert exported_model is not None, (
            "Python inference is not yet implemented for "
            f"export_method={args.export_method}, format={args.format}."
        )
        logger.info("Running 1 ... this takes a long time if you export to CPU.")
