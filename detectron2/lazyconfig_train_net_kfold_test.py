...
def do_valid(cfg, model):
    if "evaluator" in cfg.dataloader:
        if cfg.train.split_combine.enabled:
            ...
            model = SplitCombineModelWrapper(model,
                    instantiate(cfg.train.split_combine.split_combiner),
                    batch_size= cfg.dataloader.train.batch_size_val)

        ret = inference_on_dataset(
            model, instantiate(cfg.dataloader.train)[1], instantiate(cfg.dataloader.evaluator)
        )
        print_csv_format(ret)
        return ret
...
def do_train(args, cfg):
    ...
    train_loader, _ = instantiate(cfg.dataloader.train)
    ...
    trainer.register_hooks(
        [
            ...
            hooks.EvalHook(cfg.train.eval_period, lambda: do_valid(cfg, model)),
            hooks.DataloaderReset(1,cfg.dataloader.train),
...
if __name__ == "__main__":
    ...
    args.config_file = '..\LSIVesSeg\configs\\baseline.py'
    # args.resume = True
    # args.eval_only = True
    launch(
        main,
