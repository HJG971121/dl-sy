...
class LRSchedulerGAN(HookBase):
    ...
    def before_train(self):
        self._optimizer_G = self.trainer.model.optimizer_G
        self._optimizer_D = self.trainer.model.optimizer_D
        self._schedulers = []
        self._best_param_group_id = []
        if isinstance(self.scheduler, ParamScheduler):
            for idx, optimizer in enumerate([self._optimizer_G, self._optimizer_D]):
                self._schedulers.append(
                    LRMultiplier(
                        optimizer,
                        copy.deepcopy(self.scheduler),
                        self.trainer.max_iter,
                        last_iter=self.trainer.iter - 1,
                    ))
                self._best_param_group_id.append(self.get_best_param_group_id(optimizer))
    ...
    def after_step(self):
        lr_G = self._optimizer_G.param_groups[self._best_param_group_id[0]]["lr"]
        self.trainer.storage.put_scalar("lr_G", lr_G, smoothing_hint=False)
        self._schedulers[0].step()

        lr_D = self._optimizer_D.param_groups[self._best_param_group_id[1]]["lr"]
        self.trainer.storage.put_scalar("lr_D", lr_D, smoothing_hint=False)
...
class EvalHook(HookBase):
    """
    Run an 1 function periodically, and at the end of training.
    ...
    def __init__(self, eval_period, eval_function, eval_after_train=True):
        ...
        Args:
            ...
            eval_function (callable): a function which takes no arguments, and
                returns a nested dict of 1 metrics.
            eval_after_train (bool): whether to evaluate after the last iteration

        Note:
            This hook must be enabled in all or none workers.
            If you would like only certain workers to perform 1,
