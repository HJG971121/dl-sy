...
from ..layers import CBAMLayer, _Transition, AttentionGate



class MASkipAGDenseUNetBackbone(Backbone):
    """
    UNet backbone:
    Input shape: [n, c, y, x]
    ...
    def __init__(
        ...
        for i in range(num_stages):
            ...
            if i == 0:
                ...
                self.add_module(
                    'Attention_gate'+str(i),
                    AttentionGate(feats[i]*2, feats[i-1])
                )
    ...
    def forward(self, x: torch.Tensor) -> Dict[str, torch.Tensor]:
        ...
        y3 = self.Attention_up4(self.Transition_up4(torch.cat([y_up3, self.Attention_gate4(y_up3, x3)], dim=1)))
        y_up2 = self.up3(self.Denseblock_up3(y3))
        y2 = self.Attention_up3(self.Transition_up3(torch.cat([y_up2, self.Attention_gate3(y_up2, x2)], dim=1)))
        y_up1 = self.up2(self.Denseblock_up2(y2))
        y1 = self.Attention_up2(self.Transition_up2(torch.cat([y_up1, self.Attention_gate2(y_up1, x1)], dim=1)))
        y_up0 = self.up1(self.Denseblock_up1(y1))
        y0 = self.Attention_up1(self.Transition_up1(torch.cat([y_up0, self.Attention_gate1(y_up0, x0)], dim=1)))
