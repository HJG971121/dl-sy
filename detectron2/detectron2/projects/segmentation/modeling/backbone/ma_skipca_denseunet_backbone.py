...
from ..layers import CBAMLayer, _Transition, MultiHeadCrossAttention



class MASkipCADenseUNetBackbone(Backbone):
    """
    UNet backbone:
    Input shape: [n, c, y, x]
        ...
        for i in range(num_stages):
            ...
            if i == 0:
                ...
                self.add_module(
                    'Transition_up'+str(i),
                    _Transition(
                        feats[i] + num_layers[i] * growth_rate[i] + feats[i-1]*2,
                        feats[i-1],
                        kernel_size=k,
                        stride=1,
                        abn=abn)
                ...
                self.add_module(
                    ...
                    CBAMLayer(feats[i], reduction=8)
                )

                self.add_module(
                    'Attention_up'+str(i),
                    CBAMLayer(feats[i-1], reduction=8)
                )

                self.add_module(
                    'CA'+str(i),
                    MultiHeadCrossAttention(feats[i]*2, feats[i-1])
                )
    ...
    def forward(self, x: torch.Tensor) -> Dict[str, torch.Tensor]:
        ...
        y_up3 = self.up4(self.Denseblock_up4(x4))
        y3 = self.Attention_up4(self.Transition_up4(torch.cat([y_up3, self.CA4(y_up3, x3)], dim=1)))
        y_up2 = self.up3(self.Denseblock_up3(y3))
        y2 = self.Attention_up3(self.Transition_up3(torch.cat([y_up2, self.CA3(y_up2, x2)], dim=1)))
        y_up1 = self.up2(self.Denseblock_up2(y2))
        y1 = self.Attention_up2(self.Transition_up2(torch.cat([y_up1, self.CA2(y_up1, x1)], dim=1)))
        y_up0 = self.up1(self.Denseblock_up1(y1))
        y0 = self.Attention_up1(self.Transition_up1(torch.cat([y_up0, self.CA1(y_up0, x0)], dim=1)))
