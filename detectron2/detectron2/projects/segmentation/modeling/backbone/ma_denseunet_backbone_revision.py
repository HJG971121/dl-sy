...
from ..layers import CBAMLayer, _Transition, Conv2dABN
...
class MADenseUNetBackbone(Backbone):
    ...
    def forward(self, x: torch.Tensor) -> Dict[str, torch.Tensor]:
        x0 = self.ConvIn(x)
        x1 = self.Attention_down1(self.down1(self.Denseblock_down1(x0)))
        x2 = self.Attention_down2(self.down2(self.Denseblock_down2(x1)))
        x3 = self.Attention_down3(self.down3(self.Denseblock_down3(x2)))
        x4 = self.Attention_down4(self.down4(self.Denseblock_down4(x3)))

        y3 = self.Attention_up4(torch.cat([self.up4(self.Denseblock_up4(x4)),x3], dim=1))
        y2 = self.Attention_up3(torch.cat([self.up3(self.Denseblock_up3(y3)),x2], dim=1))
        y1 = self.Attention_up2(torch.cat([self.up2(self.Denseblock_up2(y2)),x1], dim=1))
        y0 = self.Attention_up1(torch.cat([self.up1(self.Denseblock_up1(y1)),x0], dim=1))
